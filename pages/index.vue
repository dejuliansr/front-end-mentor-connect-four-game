<template>
  <div class="flex items-center justify-center min-h-screen p-2 sm:p-0 bg-Light-Purple sm:bg-Purple">
    ...
    <Transition name="fade-slide" mode="out-in">
      <div v-if="!showRules && !showGame && !showSoon" class="bg-Light-Purple p-10 rounded-3xl w-full max-w-md shadow-lg sm:border-4 sm:border-b-8 sm:border-black relative">
        <div class="flex justify-center py-8">
          <img src="/images/logo.svg" alt="logo">
        </div>
        <div class="space-y-5 mt-6">
          <!-- Tombol 1 -->
          <button @click="showSoon = true" class="w-full h-16 flex justify-between items-center text-xl px-4 bg-Pink text-white font-bold rounded-2xl border-2 border-black hover:border-Purple shadow-md cursor-pointer">
            PLAYER VS CPU
          ...
          <!-- Tombol 2 -->
          <button @click="showGame = true" class="w-full h-16 flex justify-between items-center text-xl px-4 bg-Yellow text-black font-bold rounded-2xl border-2 border-black hover:border-Purple shadow-md cursor-pointer">
            PLAY VS PLAYER
            <span class="flex items-center gap-2">
          ...
          <!-- Tombol 3 -->
          <button @click="showRules = true" class="w-full h-16 flex justify-start items-center text-xl px-4 bg-White text-black font-bold rounded-2xl border-2 border-black hover:border-Purple shadow-md cursor-pointer">
            GAME RULES
          </button>
        </div>
      </div>
    ...
    <Transition name="fade-slide" mode="out-in">
      <div v-if="showSoon" class="bg-White p-10 rounded-3xl w-full max-w-md shadow-lg border-4 border-b-8 border-black relative">
        <h3 class="text-Black font-bold text-3xl mb-3 text-center">Coming Soon</h3>
        <div class="absolute left-1/2 transform -translate-x-1/2">
          <!-- Tombol Ceklis untuk Kembali ke Menu -->
          <button @click="showSoon = false" class="mx-auto flex justify-center items-center bg-Pink text-white hover:border-Purple rounded-full shadow-md cursor-pointer">
            <img src="/images/icon-check.svg" alt="icon Check" class="hover:border-Purple">
          </button>
        </div>
    ...
    <GameBoard @back-to-menu="showGame = false" v-if="showGame" class="w-full" />
...
const showSoon = ref<boolean>(false);
</script>
